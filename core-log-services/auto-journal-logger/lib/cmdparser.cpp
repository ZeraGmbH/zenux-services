    ...
    QSimpleCmdParserSocketBase(parent)
{
    AddCmdInfo("SaveLogAndDumps",
               CmdParamTypeIdList() << PARAM_TYPE_STRING << PARAM_TYPE_STRING << PARAM_TYPE_STRING,
               CMD_SAVE_LOG_AND_DUMPS,
               true,
               QStringLiteral("Parameter: DestDir,VersionInfoFilePath(empty allowed),UpdateLogsSourceDir(empty allowed)\nReturn: Log status info"));
}
...
const QString CmdParser::PlausiCheck(SimpleCmdData *pCmd, const QVariantList &params)
{
    ...
    case CMD_SAVE_LOG_AND_DUMPS:
        ...
        if(path.isEmpty())
            errInfo.append(QStringLiteral("Parameter DestDir cannot be empty!"));
        else {
