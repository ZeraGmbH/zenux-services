...
AtmelCriticalStatusPtr MockFactoryI2cCtrl::getCriticalStatusController()
{
    return std::make_unique<MockI2cCtrlCriticalStatus>(m_persitentData.m_criticalStatus,
                                                     m_persitentData.m_criticalStatusMask);
}

AtmelPermissionTemplatePtrU MockFactoryI2cCtrl::getPermissionCheckController()
{
    return std::make_unique<MockI2cCtrlEepromPermission>(m_persitentData.m_permission);
}

AtmelCommonVersionsPtrU MockFactoryI2cCtrl::getCommonVersionController(ControllerTypes ctrlType, quint8 muxChannel)
{
    Q_UNUSED(ctrlType)
    Q_UNUSED(muxChannel)
...
AtmelDeviceIdentificationDataU MockFactoryI2cCtrl::getDeviceIdentificationController()
{
...
AtmelAccumulatorHandlerPtrU MockFactoryI2cCtrl::getAccumulatorController()
{
    return std::make_unique<MockI2cCtrlAccumulator>();
}

AtmelRangesPtrU MockFactoryI2cCtrl::getRangesController()
{
    return std::make_unique<MockI2cCtrlRanges>();
}

AtmelMModesPtrU MockFactoryI2cCtrl::getMModeController()
{
    return std::make_unique<MockI2cCtrlMMode>();
}

AtmelPllPtrU MockFactoryI2cCtrl::getPllController()
{
