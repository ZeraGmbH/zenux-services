...
FInGroupResourceAndInterface::FInGroupResourceAndInterface(const std::shared_ptr<cSCPI> &scpiInterface,
...
void FInGroupResourceAndInterface::initSCPIConnection()
{
    addDelegate("FRQINPUT", "VERSION", SCPI::isQuery, m_scpiInterface, cmdVersion);
    addDelegate("FRQINPUT:CHANNEL", "CATALOG", SCPI::isQuery, m_scpiInterface, cmdChannelCat);
    for (auto channel : qAsConst(m_ChannelList)) {
        ...
        channel->initSCPIConnection("FRQINPUT");
    }
}
...
void FInGroupResourceAndInterface::executeProtoScpi(int cmdCode, const ProtonetCommandPtr &protoCmd)
{
    switch (cmdCode)
    {
    case cmdVersion:
...
QString FInGroupResourceAndInterface::readVersion(const QString &scpi)
{
...
QString FInGroupResourceAndInterface::readChannelCatalog(const QString &scpi)
{
    cSCPICommand cmd = scpi;
    if (cmd.isQuery()) {
        int i;
