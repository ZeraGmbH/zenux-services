...
cHKeySettings::cHKeySettings(Zera::XMLConfig::cReader *xmlread)
{
    ...
    for (int i = 0; i < 1; i++) {
        ...
        m_ConfigXMLMap[QString("serviceconfig:resource:hkey:hk%1:alias").arg(i)] = HKeySystem::cfgHK0Alias + i;
        m_ConfigXMLMap[QString("serviceconfig:resource:hkey:hk%1:avail").arg(i)] = HKeySystem::cfgHK0avail + i;
    }
}
