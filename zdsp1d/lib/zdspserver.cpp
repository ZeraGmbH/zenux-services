...
void ZDspServer::DspIntHandler(int)
{
    ...
    if (!clientList.isEmpty()) { // wenn vorhanden nutzen wir immer den 1. client zum lesen
        ...
        QByteArray ba;
        if (m_dspInOut.readOneDspVar(QString("CTRLCMDPAR,%1").arg(DSP_INTERRUPT_PARAM_BUFFER_LEN),
                                                              ...
                                                              bool &ok) const
{
    ...
    return compiler.compileOneCmdLine(QString("USERMEMOFFSET(%1)").arg(userMemOffset), compilerSupport, ok);
}
                                                            ...
                                                            QByteArray &rawInterruptCmdMemOut) const
{
    ...
    QDataStream cycCmdMemStream(&rawCyclicCmdMemOut, QIODevice::Unbuffered | QIODevice::ReadWrite);
